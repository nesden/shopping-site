...
import { addItemsToOrder, findAllItems, getAllOrdersByUser, search, addToFavorites, removeFromFavorites, getAllFavorites } from '../services/ApiService';
...
const MyFavorites = () => {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    if (!searchBar.trim()) {
      const { data } = await getAllFavorites(); // reset if empty
      setItems(data);
      return;
    }
    try {
  ...
  const handleAddItem = async ({ itemId }) => {
    try {
      ...
      const { data } = await getAllFavorites(); // ✅ Refresh with updated amounts
      setItems(data); // ✅ Refresh displayed items
